export default function madLibsArray(movies) {
  ...
  const randomCastSingle = () => {
    let randomCast = movies.cast;
    let randomNumber = Math.floor(Math.random() * randomCast.length);
    if (randomCast[randomNumber].known_for_department === "Acting" ) {
      return randomNumber;
    }
  }
  ...
  return (
    movies && [
      ...
      // },
      {
        question: `${movies.cast[randomCastSingle()].name} starred in ${movies.title}.`, //get credits endpoint for performer name
        answer: true,
        movieId: `${movies.id}`,
      },
      // {
